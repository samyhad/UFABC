...
public class Main
{
    ...
    public static void exerc2(){
        ...
        for(int  i = 1; i <= 3*n; i++){
            ...
            lambda_anterior = -2 + (double)(4*(i-1))/(3*n);
            lambda_atual = -2 + (double)(4*i)/(3*n);
            
            if((fTGn(lambda_atual, n, debug)*fTGn(lambda_anterior, n, debug)) < 0){
                System.out.println("[" + lambda_anterior +", "+ lambda_atual+"]");
            }
    ...
    public static void exerc4(){
        ...
        Double[] intervalo = {-1.9, -1.7, -1.4, 0.0, 1.0, 1.5, 2.0};
        int max = intervalo.length;
        int n = 11;
        double lambda = 0;
        double derivada = 0;
        ...
        for(int j = 0; j < max; j++){
            ...
            lambda = intervalo[j];
            derivada = fLinha(lambda, n);
            System.out.println("Derivada de fTGn("+lambda+") é igual a "+ derivada);
